...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        x0, cache2 = word_embedding_forward(captions_in, W_embed)
        if self.cell_type == "rnn":
            h_, cache3 = rnn_forward(x0, h0, Wx, Wh, b)
        else:
            h_, cache3 = lstm_forward(x0,h0, Wx, Wh, b)
        x, cache4 = temporal_affine_forward(h_, W_vocab,b_vocab)
        loss, dL = temporal_softmax_loss(x,captions_out, mask)

        dx, dW_vocab, db_vocab = temporal_affine_backward(dL, cache4)
